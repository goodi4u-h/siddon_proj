...
from ray.unit_geo import Voxel, Detector, Point2D
...
class Geometry(object):
    ...
    def check_geo(self, angles, verbose=False):
        ...
        ## Check the dimension of the given angles
        if angles.ndim == 1:
        ...
        # image data
        if not self.nVoxel.shape == (2,):
            raise AttributeError("geo.nVoxel.shape should be (2, )")
        if not self.sVoxel.shape == (2,):
            raise AttributeError("geo.sVoxel.shape should be (2, )")
        if not self.dVoxel.shape == (2,):
            raise AttributeError("geo.dVoxel.shape should be (2, )")
        ...
        if not sum(abs(
            self.dDetector * self.nDetector - self.sDetector
        )) < 1e-6:
            raise AttributeError(
    ...
    def __check_and_repmat__(self, attrib, angles):
        ...
        if type(old_attrib) in [float, int, np.float32, np.float64, np.int32]:
            ...
            if old_attrib.ndim == 1:
                if old_attrib.shape in [(3,), (2,)] \
                    and attrib not in ["DSD", "DSO", "COR"]:
    ...
    def __str__(self):
        ...
        parameters = []
        detector_info = [
            self.nDetec.__dict__,
            self.dDetec.__dict__,
            self.sDetec.__dict__
        ]
        image_info = [
            self.nVoxel.__dict__,
            self.sVoxel.__dict__,
            self.dVoxel.__dict__
        ]
        ...
        parameters.append(
            "Distance from source to origin (DSO)= " + \
            str(self.DSO) + \
            " mm"
        )
        ...
        parameters.append(
            "Number of voxels (nVoxel) = " + \
            f"{image_info[0]}"
        )
        parameters.append(
            "Total size of the image (sVoxel) = " + \
            f"{image_info[1]}" + \
            " mm"
        )
        parameters.append(
            "Size of each voxel (dVoxel) = " + \
            f"{image_info[2]}" + \
            " mm"
        )
        ...
        if hasattr(self, "offOrigin") and hasattr(self, "offDetector"):
            ...
            if hasattr(self, "offOrigin"):
                parameters.append(
                    "Offset of image from origin (offOrigin) = " \
                    + str(self.offOrigin.__dict__) + \
                    " mm"
                )
            if hasattr(self, "offDetector"):
                parameters.append(
                    "Offset of detector (offDetector) = " + \
                    str(self.offDetector.__dict__) + \
                    " mm"
                )
        ...
        parameters.append(
            "Samples per pixel of forward projection (accuracy) = " + \
            str(self.accuracy)
        )
        ...
        if hasattr(self, "rotDetector"):
            parameters.append("-----")
            parameters.append(
                "Rotation of the Detector (rotDetector) = " + \
                str(self.rotDetector.__dict__) + \
                " rad"
            )
        ...
        if hasattr(self, "COR"):
            parameters.append("-----")
            parameters.append(
                "Centre of rotation correction (COR) = " + \
                str(self.COR) + \
                " mm"
            )
...
class ParallelGeo(Geometry):
    def __init__(self, nVoxel:tuple, nDetec:int):  # noqa: N803
        if nVoxel is None:
            raise ValueError(
                "nVoxel needs to be given for initialisation of parallel beam"
            )
        ...
        self.nVoxel = Voxel(dtype='n', x=nVoxel[0], y=nVoxel[1])
        self.sVoxel = Voxel(dtype='s', x=self.nVoxel.x, y=self.nVoxel.y)
        self.dVoxel = Voxel(
            dtype='d',
            x=self.nVoxel.x/self.sVoxel.x,
            y=self.nVoxel.y/self.sVoxel.y
        )

        self.nDetec = Detector(dtype='n', u=nDetec)
        self.dDetec = Detector(dtype='d', u=1.0)
        self.sDetec = Detector(dtype='s', u=self.nDetec.u*self.dDetec.u)

        self.DSO = np.float64(self.nVoxel.x*2)
        self.DSD = np.float64(self.nVoxel.x*4)

        self.accuracy:np.float64 = 0.5

        self.COR = np.float64(0)

        self.offOrigin = Point2D(x=0, y=0)
        self.offDetector = Detector(dtype='s', u=0)

        self.rotDetector = Point2D(x=0.0, y=0.0)
        self.dRoll = np.float64(0)
        self.dPitch = np.float64(0)
        self.dYaw = np.float64(0)


class ConeGeo(Geometry):
    ...
    def __init__(self, nVoxel:tuple, nDetec:int):  # noqa: N803
        ...
        self.dVoxel = Voxel(
            dtype='d',
            x=self.sVoxel.x/self.nVoxel.x,
            y=self.sVoxel.y/self.nVoxel.y
        )

        self.nDetec = Detector(dtype='n', u=nDetec)
        self.dDetec = Detector(dtype='d', u=1.0)
        self.sDetec = Detector(dtype='s', u=self.dDetec.u*self.nDetec.u)
